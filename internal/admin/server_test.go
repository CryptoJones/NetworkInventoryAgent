...
func newTestServer(t *testing.T, hosts *mockHostStore, ports *mockPortStore, scans *mockScanStore) *admin.Server {
	t.Helper()
	srv, err := admin.NewServer(":0", "test-agent", hosts, ports, scans, healthyStatus)
...
func TestNewServer_ParsesTemplates(t *testing.T) {
	_, err := admin.NewServer(":0", "agent", &mockHostStore{}, &mockPortStore{}, &mockScanStore{}, healthyStatus)
...
func TestServer_Shutdown(t *testing.T) {
	srv, err := admin.NewServer(":0", "agent", &mockHostStore{}, &mockPortStore{}, &mockScanStore{}, healthyStatus)
	require.NoError(t, err)
	require.NoError(t, srv.Start())
